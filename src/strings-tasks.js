...
function removeFirstOccurrences(str, value) {
  ...
  return indexValue >= 0 ? newString : str;
}
...
function formatTime(minutes, seconds) {
  const updatedMinutes = `${minutes}`.padStart(2, '0');
  const updatedSeconds = `${seconds}`.padStart(2, '0');
...
function isPalindrome(str) {
  const updatedString = str.replace(/[\s.,!?]/g, '').toLowerCase();
  for (let i = 0; i <= Math.floor(updatedString.length / 2); i += 1) {
...
function reverseWords(str) {
  ...
  const updatedArray = array.map((item) => item.split('').reverse().join(''));
...
function extractNameFromTemplate(value) {
  const empty = value.indexOf(' ');
  const sign = value.indexOf('!');
  return value.slice(empty + 1, sign);
}
